 ...
 *
 */
public class CheckProcessingTaskThread extends Thread {

	private static final Log LOGGER = LogFactory.getLog(CheckProcessingTaskThread.class);

	/**
	 * 
	 */
	public CheckProcessingTaskThread() {
	}
	...
	public void run() {
		...
		while (true) {
			try {
				...
				List<ReaperTask> ts = RabbitMQUtils.getProcessingTask(64);
				...
				if (null == ts || ts.size() == 0) {
					...
					for (int i = 0; i < ts.size(); i++) {
						...
						if (null == task || task.isInvalid()) {
							...
							bean.setStatus(1l);
							bean.setProcessHost(task.getHost());
